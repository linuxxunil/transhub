...
TransHub 是一個桌面版 Chrome / Edge（Manifest V3）劃詞翻譯擴展：在閱讀英文網站時，**選中文字後連按兩次快速鍵（預設 `T`）**，即可即時翻譯為繁體中文；也支持開啟**雙擊翻譯**。它聚合多家翻譯服務為一個入口——某家免費額度用盡時自動切換下一家，全部用盡即停止，**絕不自動產生付費**。
...
| 操作 | 行為 |
|---|---|
| 選中文字後连按兩次快速鍵（預設 `T`，可自定義） | 翻譯選中內容（單詞、句子均可） |
| 雙擊單詞或已選取文字（設置頁開啟，預設關閉） | 翻譯單詞／選中內容 |
| 先選中文字 → 右鍵菜單「翻譯選中文本」 | 翻譯選中內容 |
| popup「翻譯 PDF」/ 右鍵 PDF 連結「用 TransHub 翻譯此 PDF」 | 內建檢視器開啟 PDF，劃詞即時翻譯 |
| Esc / 點擊卡片外空白 | 關閉翻譯卡片 |
...
- 快速鍵防誤觸：輸入框/textarea 內不觸發、带修飾鍵不觸發、按住不放不觸發
- **PDF 劃詞翻譯**：內建 PDF.js 檢視器（本地打包，無遠端代碼），開啟本機或當前分頁 PDF 後雙擊/選取即譯；Chrome/Edge 內建 PDF 檢視器因平台限制無法翻譯，跨網域 PDF 連結可能需先下載
...
| 阿里雲機器翻譯 | 每月 100 萬字符（通用版/專業版各 100 萬） | 超額進入後付費，注意關閉或設餘額提醒。繁體目標僅支援英語/自動檢測來源，日韓來源會自動切換其他服務 |
...
5. 打開任意英文網頁，選中一段文字後連按兩次 `T` 試試（或在設置頁開啟「雙擊觸發翻譯」後雙擊單詞）
...
├── content.js           # 初始化共享劃詞模組（頁面交互）
├── lib/selection-card.js # 共享模組：雙擊/快速鍵觸發、浮動卡片（Shadow DOM）
├── pdf-viewer.html/.js  # 內建 PDF.js 檢視器：本機/當前分頁 PDF 劃詞翻譯
├── lib/pdfjs/           # 本地打包的 PDF.js（無遠端代碼）
