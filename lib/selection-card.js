/**
 * TransHub 共享模組：劃詞翻譯浮動卡片與觸發邏輯
 * 由 content script（lib/selection-card.js → content.js，見 manifest content_scripts 順序）
 * 與 PDF 檢視器頁面（pdf-viewer.html）共用，確保兩處行為一致。
 * 掛載點：global.TransHubCard.init()
 */
(function (global) {
  'use strict';
  ...
  function startTranslate(text, x, y) {
    ...
    if (!contextValid()) {
      renderResult({ ok: false, error: invalidMsg() });
      return;
    }
    try {
      ...
      renderResult({ ok: false, error: contextValid() ? (e && e.message || String(e)) : invalidMsg() });
    }
  }

  var hotkeyCfg = { enabled: true, key: 't', windowMs: 600 };
  var dblclickEnabled = false;
  ...
  function loadTriggerCfg() {
    if (!contextValid()) return;
    try {
      chrome.storage.local.get({ settings: null }, function (res) {
        var s = res.settings;
        if (!s) return;
        if (typeof s.dblclickEnabled === 'boolean') dblclickEnabled = s.dblclickEnabled;
        if (!s.hotkey || typeof s.hotkey !== 'object') return;
        hotkeyCfg = {
    ...
    if (shadowHost.contains(e.target)) {
      ...
      if (btn && card) {
        ...
        if (act === 'close') hideCard();
          ...
          navigator.clipboard.writeText(lastResult.text).catch(function () {});
          btn.textContent = '已複製';
  ...
  function onKeydown(e) {
    ...
    if (lastTap.key === k && now - lastTap.time <= hotkeyCfg.windowMs) {
      ...
      var r = selectionRect();
      if (!text && lastMouse.x >= 0) {
        text = wordAtPoint(lastMouse.x, lastMouse.y);
        text = (text || '').trim();
      }
      if (!text || text.length > 1000) return;
      e.preventDefault();
      var x = r ? Math.min(r.right, window.innerWidth - 30) : Math.min(lastMouse.x, window.innerWidth - 30);
      var y = r ? Math.min(r.bottom + 8, window.innerHeight - 100) : Math.min(lastMouse.y + 14, window.innerHeight - 100);
    ...
    init: function () {
      ...
      if (chrome.storage.onChanged) {
        chrome.storage.onChanged.addListener(function (changes, area) {
          if (area === 'local' && changes.settings) loadTriggerCfg();
        });
      }
