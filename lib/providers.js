  ...
  var providers = {
    ...
    alibaba: {
      ...
      translate: function (text, source, target, cfg) {
        ...
        var stringToSign = 'POST&%2F&' + pct(canonical);
        return hmacSha1Raw(cfg.accessKeySecret + '&', stringToSign).then(function (sig) {
          var bytes = new Uint8Array(sig);
          var bin = '';
          for (var i = 0; i < bytes.length; i++) bin += String.fromCharCode(bytes[i]);
          var b64 = btoa(bin);
          return fetchWithTimeout('https://mt.cn-hangzhou.aliyuncs.com/', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' },
            body: canonical + '&Signature=' + pct(b64)
          }).then(function (r) {
            if (!r.ok) {
              return r.json().catch(function () { return null; }).then(function (errData) {
                if (errData && errData.Code) bail('阿里雲：' + errData.Code + ' ' + (errData.Message || ''));
          ...
          if (code && code !== '200') bail('阿里雲：' + code + ' ' + (data.Message || ''));
